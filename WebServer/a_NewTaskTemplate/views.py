...
# task's own imports
from .NewTaskTemplateAnalysis import *
# from .NewTaskTemplatePropertiesAnalysis import *
from .NewTaskTemplateResult import *
# from .NewTaskTemplatePropertiesResult import *
...
def analysis_page(request):
    context = Context({
        'task_type': NEWTASKTEMPLATE_TASK,
    })
    return HttpResponse(get_template("a_NewTaskTemplate/networkProperties/analysis.html").render(context))
...
def submit_analysis_properties(request):
    ...
    rendered_view = get_template("a_NewTaskTemplate/networkProperties/result.html").render(context)
...
def submit_analysis_backend(request):
    ...
    try:
        ...
        task = NewTaskTemplateAnalysis(networks, request_FILES, task_name=task_name, user=request.user, max_iter=max_iter, delta_min=delta_min, ks=ks)
        task.submit()
        data = json.dumps({
...
def get_view_for_task_other(task, user):
    ...
    return HttpResponse(get_template("a_NewTaskTemplate/networkProperties/result.html").render(context))
