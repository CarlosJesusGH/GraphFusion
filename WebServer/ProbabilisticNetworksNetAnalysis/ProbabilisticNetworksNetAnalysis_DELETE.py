...
from .ProbabilisticNetworksNetAnalysisResult_DELETE import ProbabilisticNetworksNetAnalysisResult
from .settings import COMPUTATIONS_DIR, PROBABILISTIC_NETWORKS_NET_ANALYSIS_TASK, NAMES_MAPPINGS_FILE
...
def get_network_probabilistic_analysis_for_graphs(graphs, user, task_name):
    LOGGER.info("Starting parallel analysis")
    network_analysis_runnable = ProbabilisticNetworksNetAnalysis(graphs=graphs, user=user, task_name=task_name)
...
class ProbabilisticNetworksNetAnalysis(Runnable):
    ...
    def __initialise_task(cls, task_name, temp, user):
        task = Task()
        task.task_type = PROBABILISTIC_NETWORKS_NET_ANALYSIS_TASK
    ...
    def run(self):
        try:
            ...
            for graph_name, graph_data in self.graphs:
                network = ProbabilisticNetworksNetAnalysisResult(graph_name=graph_name + str(i), operational_dir=self.operational_directory)
