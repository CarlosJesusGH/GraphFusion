...
def saveDistanceMatrix(matrix, networkNames, outputFile):
    LOGGER.info("Writing matrix to " + str(outputFile))
...
def computeRGFDistances(graphletCounts, outputName):
    ...
    for i in range(len(graphletCounts) - 1):
        ...
        if pairCount % 1000 == 0:
            LOGGER.info('RGF Comparison Jobs submitted: ', str(float(pairCount) / jobCount * 100), '%')
    ...
    while finishedCount < pairCount:
        ...
        if finishedCount % 1000 == 0:
            LOGGER.info(('RGF Distance comparisons finished: ', str(float(finishedCount) / jobCount * 100), '%'))
...
class FinishedConsumer(multiprocessing.Process):
    ...
    def run(self):
        ...
        while finishedCount < self.jobCount:
            ...
            if finishedCount % 1000 == 0:
                LOGGER.info(('Computation completion: ', str(float(finishedCount) / self.jobCount * 100), '%'))
...
def computeNetworkProperties(allIndexes, mode):
    ...
    for index in allIndexes:
        ...
        if submitCount % 100 == 0:
            LOGGER.info(('Network Property Jobs Submitted:', str(float(submitCount) / jobCount * 100), '%'))
    ...
    while finishedCount < len(allIndexes):
        ...
        if finishedCount % 100 == 0:
            LOGGER.info(('Ready Network Properties:', str(float(finishedCount) / jobCount * 100), '%'))
...
def compareNetworkProps(propertyList, distanceMode, outputNames):
    ...
    for i in range(len(propertyList) - 1):
        ...
        for j in range(i + 1, len(propertyList)):
            ...
            if pairCount % 1000 == 0:
                LOGGER.info('Jobs submitted:', str(float(pairCount) / jobCount * 100), '%')
    ...
    while finishedCount < pairCount:
        ...
        if finishedCount % 1000 == 0:
            LOGGER.info(('Finished comparisons: ', str(float(finishedCount) / jobCount * 100), '%'))
...
def getSpectralSignatures(allIndexes):
    ...
    for index in allIndexes:
        ...
        if submitCount % 100 == 0:
            LOGGER.info(('Spectrum Computation Jobs Submitted:', str(float(submitCount) / jobCount * 100), '%'))
    ...
    while finishedCount < len(allIndexes):
        ...
        if finishedCount % 100 == 0:
            LOGGER.info(('Ready Spectrums:', str(float(finishedCount) / jobCount * 100), '%'))
...
def checkInput(ndumpFolder, distType, num_processes):
    if not os.path.isdir(ndumpFolder):
        LOGGER.info('ERROR: Provided network folder path is not found: {0}'.format(ndumpFolder))
        exit(0)
    ...
    if distType == 'rgf':
        ...
        LOGGER.info('ERROR: Unknown network distance type: {0}'.format(distType))
        LOGGER.info('Available options are:')
        LOGGER.info('rgf			- RGF distance')
        LOGGER.info('gdda			- GDD Agreement (Both Arithmetic \& Geometric)')
        LOGGER.info('degree 		- Degree distribution & Average degree distances')
        LOGGER.info('clustering	- Clustering Coefficient')
        LOGGER.info('diameter 	- Diameter')
        LOGGER.info('gcd11  		- Graphlet Correlation distance with non-redundant 2-to-4 node graphlet orbits')
        LOGGER.info('gcd15  		- Graphlet Correlation distance with all 2-to-4 node graphlet orbits')
        LOGGER.info('gcd58  		- Graphlet Correlation distance with non-redundant 2-to-5 node graphlet orbits')
        LOGGER.info('gcd73  		- Graphlet Correlation distance with all 2-to-5 node graphlet orbits')
        LOGGER.info(
            'spectral		- Spectral distance using the eigenvalues of the Laplacian representation of the network')
        exit(0)
        ...
        for fileName in file[2]:
            if fileName.endswith('.ndump2'):
                ...
                if testMode in [4, 5, 6, 17] and stripName + '.gw' not in file[2]:
                    LOGGER.info('ERROR: Network file - Ndump file mismatch!')
                    LOGGER.info('Check: {0}.gw'.format(indexName))
                    exit(0)
...
def run(allIndexes, distType, ndumpFolder, num_processes):
    ...
    if testMode in [7, 10, 14, 16]:
        ...
        LOGGER.info('GCMs are ready! Computing the GCDs for testMode: ' + str(testMode))
    ...
    # Compute the spectral distances
        ...
        LOGGER.info('Spectral Signatures are ready! Computing the distances...')
    ...
    # Compute the RGF Distance
        ...
        LOGGER.info('Graphlet Counts ready! Computing the distances...')
    ...
    # Compute the GDD-Agreement with Arithmetics and Geometric mean
        ...
        LOGGER.info('Graphlet Distributions ready! Computing the distances...')
    ...
    # Compute the degree based distance measures
        ...
        LOGGER.info('Degree Distributions ready! Computing the distances...')
    ...
    # Compute the clustering coefficient based distance measures
        ...
        LOGGER.info('Clustering Coefficients ready! Computing the distances...')
    ...
    # Compute the diameter based distance measures
        ...
        LOGGER.info('Diameters ready! Computing the distances...')
...
if __name__ == "__main__":
    ...
    run(allIndexes=allIndexes, distType=distType, ndumpFolder=ndumpFolder, num_processes=num_processes)