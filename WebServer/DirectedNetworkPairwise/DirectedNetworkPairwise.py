...
from .settings import DGDV_SIGNATURES_SCRIPT, PAIRWISE_ANALYSIS_COMPUTATIONS_DIR, DIRECTED_NETWORK_PAIRWISE_TASK, NAMES_MAPPING_FILE, \
    DN_COMPARISON_SCRIPT, DISTANCES, NAMES_OF_NETWORKS_LIST_FILES
...
def get_runnable_for_comparison_analysis(operational_dir, graph_paths, distances):
    ...
    for analysis in distances:
        if analysis not in __ALL_DISTANCES:
            raise DirectedNetworkPairwiseException(
...
class DirectedNetworkPairwiseException(Exception):
...
class DGDVRunnable(Runnable):
    def __init__(self, dgdv_executable):
        Runnable.__init__(self)
        self.dgdv_executable = dgdv_executable

    def run(self):
        self.dgdv_executable.run()


class DirectedNetworkPairwise(Task):
    ...
    def save_task_began(self, user, task_name, directory):
        self.task.task_type = DIRECTED_NETWORK_PAIRWISE_TASK
    ...
    def run_dgdv_signatures_for_all_networks_in_dir(self):
        runnable = []
        for graph_path in self.graph_paths:
            print("for graph_path:", graph_path)
            r = DGDVRunnable(dgdv_executable=DGDVExecutable(file_path=graph_path))
            runnable.append(r)
    ...
    def run_analysis(self):
        ...
        self.save_graphs()
        self.run_dgdv_signatures_for_all_networks_in_dir()
...
class DGDVExecutable:
    ...
    def run(self):
        LOGGER.info("Running DGDV for " + str(self.file_path))
        result = make_system_call(DGDV_SIGNATURES_SCRIPT + ' ' + self.file_path)
        if result.return_code != 0:
            raise DGDVRunnerException("Error while running parallel runner for DGDV: " + str(result.stderr))
        # self.remove_computational_files()


class DGDVRunnerException(Exception):
    def __init__(self, message):
        Exception.__init__(self, message)