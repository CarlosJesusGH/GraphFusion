...
# task's own imports
from .GraphletLaplaciansAnalysis import *
from .GraphletLaplaciansResult import *
...
def analysis_page(request):
    context = Context({
        'task_type': GraphletLaplacians_TASK,
    })
    return HttpResponse(get_template("GraphletLaplacians/networkProperties/analysis.html").render(context))
...
def submit_analysis_properties(request):
    ...
    rendered_view = get_template("GraphletLaplacians/networkProperties/result.html").render(context)
...
def analysis_other_like_MultAlign(request):
    ...
    try:
        ...
        task = GraphletLaplaciansAnalysis(networks, request_FILES, task_name=task_name, user=request.user, max_iter=max_iter, delta_min=delta_min, ks=ks)
        task.submit()
        data = json.dumps({
...
def get_view_for_task_other(task, user):
    ...
    return HttpResponse(get_template("GraphletLaplacians/networkProperties/result.html").render(context))
