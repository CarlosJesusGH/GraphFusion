...
def Creat_Network(Network_Name, save_directory, network_path):
    
    original_pd = pd.read_csv(f'{network_path}{Network_Name}', sep = " ")
    ...
    gene2int_df.to_csv(f'{save_directory}Gene_Names_{Network_Name}')
...
def Compute_Orbits_Spliting_Nodes(Network_Name, Total_Number_Nodes, file_Directory, save_directory, Run_path, undir_path, threads = 1):
    
    os.chdir(undir_path) # this has to be the path to undir3 and undir4 file.
    ...
    for sentence in range(len(groups)):
        ...
        sub_command_G4 = [str(Run_path) + "Lucky_GOGO_Node_Parallel -s 4" + " "  +
                             "-m gtrie " +str(file_Directory)+ "undir4.gt " + " " +
                             " -g" + " " +str(file_Directory)  + str(Network_Name) +
