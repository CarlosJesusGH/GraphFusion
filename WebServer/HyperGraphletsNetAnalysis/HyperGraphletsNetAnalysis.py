...
class HyperGraphletsNetAnalysisException(Exception):
    def __init__(self, message):
        Exception.__init__(self, message)

class HyperGraphletsNetAnalysis(Task):
    ...
    def __save_task_began(self, user, task_name, directory):
        ...
        self.task.task_type = HYPERGRAPHLETS_NET_ANALYSIS_TASK
    ...
    def __run_task(self):
        ...
        os.system("pwd")
        os.system("cp HyperGraphletsNetAnalysis/scripts/hypergraphlet_counter/source/hypergraphlets_1_4_map " + self.operational_dir)
        LOGGER.info("Running HDV for " + self.net_names[0][0])
        make_system_call("pwd")
        result = make_system_call("../../scripts/hypergraphlet_counter/source/run_hypercounter -g " + self.net_names[0][0] + " -o output.hdv -t 1 -b 50", working_dir=self.operational_dir)
        os.remove(self.operational_dir + "hypergraphlets_1_4_map")
        if result.return_code != 0:
            raise HDVRunnerException("Error while running parallel runner for HDV: " + str(result.stderr))
    ...
    def run_task(self):
        try:
            ...
            LOGGER.info(result)
            if result and len(result.stderr):
                print("len(result.stderr)", len(result.stderr))
                # print("result.stderr", result.stderr)
                raise HyperGraphletsNetAnalysisException("Error occurred while computing task: \n\n" + result.stderr)
...
class HDVRunnerException(Exception):
    def __init__(self, message):
        Exception.__init__(self, message)