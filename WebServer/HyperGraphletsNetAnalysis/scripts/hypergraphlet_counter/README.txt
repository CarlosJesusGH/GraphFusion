...
The counter outputs a .svml format count with the following format: - each row correspond to the count for a vertex - the first number (and last) correspond to the vertex index - the count for each orbit is of the shape orbit:count
...
If in need of recompiling, you need to change the path to the local installation of boost in the Makefile, then just run make
