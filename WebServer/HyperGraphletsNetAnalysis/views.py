...
def analysis_page(request):
    context = Context({
        'task_type': HYPERGRAPHLETS_NET_ANALYSIS_TASK,
    })
    return HttpResponse(get_template("HyperGraphlets/netAnalysis/analysis.html").render(context))
...
def submit_analysis(request):
    ...
    try:
        ...
        task = HyperGraphletsNetAnalysis(networks, request_FILES, task_name=task_name, user=request.user)
        task.submit()
        data = json.dumps({
...
def get_view_for_task(task, user):
    ...
    return HttpResponse(get_template("HyperGraphlets/netAnalysis/result.html").render(context))
