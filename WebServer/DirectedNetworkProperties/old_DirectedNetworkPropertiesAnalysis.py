...
class DirectedNetworkPropertiesAnalysis(Runnable):
    def __init__(self, graphs, user, task_name):
        ...
        while os.path.isdir(DIRECTED_NETWORK_PROPERTIES_COMPUTATIONS_DIR + "/" + temp):
            temp = uuid.uuid4().hex
        self.operational_directory = DIRECTED_NETWORK_PROPERTIES_COMPUTATIONS_DIR + "/" + temp
    ...
    def __initialise_task(cls, task_name, temp, user):
        task = Task()
        task.task_type = DIRECTED_NETWORK_PROPERTIES_TASK
    ...
    def run(self):
        try:
            ...
            for graph_name, graph_data in self.graphs:
                network = DirectedNetworkProperties(graph_name=graph_name + str(i), operational_dir=self.operational_directory)
            ...
            with open(self.operational_directory + "/" + DIRECTED_NETWORKS_NAMES_MAPPINGS_FILE_NAME, "w") as f:
