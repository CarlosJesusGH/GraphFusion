...
from .DirectedNetworkProperties import DirectedNetworkProperties
...
class DirectedNetworkPropertiesAnalysis(Task):
    def __init__(self, graphs, user, task_name):
        ...
        self.operational_dir = DIRECTED_NETWORK_PROPERTIES_COMPUTATIONS_DIR + "/" + temp + "/"
    ...
    def __get_fresh_dir(cls):
        temp = uuid.uuid4().hex
        while os.path.isdir(DIRECTED_NETWORK_PROPERTIES_COMPUTATIONS_DIR + "/" + temp):
            temp = uuid.uuid4().hex
        return temp
    ...
    def __save_task_began(self, user, task_name, directory):
        ...
        self.task.task_type = DIRECTED_NETWORK_PROPERTIES_TASK
    ...
    def run_task(self):
        try:
            ...
            for graph_name, graph_data in self.graphs:
                network = DirectedNetworkProperties(graph_name=graph_name + str(i), operational_dir=self.operational_dir)
            ...
            with open(self.operational_dir + "/" + DIRECTED_NETWORKS_NAMES_MAPPINGS_FILE_NAME, "w") as f:
                f.write(str(mappings))
            # Compute the 'rendered_view' also in the background
            from django.template import Context
            from django.template.loader import get_template
            from .settings import DIRECTED_NETWORK_RESULT_VIEW_FILE_NAME
