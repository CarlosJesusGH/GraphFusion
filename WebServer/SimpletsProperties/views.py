...
# task's own imports
from .SimpletsPropertiesAnalysis import *
from .SimpletsPropertiesResult import *
...
def analysis_page(request):
    context = Context({
        'task_type': SIMPLETS_PROPERTIES_TASK,
    })
    return HttpResponse(get_template("Simplets/networkProperties/analysis.html").render(context))
...
def submit_analysis_properties(request):
    ...
    rendered_view = get_template("Simplets/networkProperties/result.html").render(context)
...
def get_view_for_task_other(task, user):
    ...
    return HttpResponse(get_template("Simplets/networkProperties/result.html").render(context))
