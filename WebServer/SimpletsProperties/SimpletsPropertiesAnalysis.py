...
from .SimpletsPropertiesResult import SimpletsPropertiesResult
from .settings import COMPUTATIONS_DIR, SIMPLETS_PROPERTIES_TASK, NAMES_MAPPINGS_FILE
...
def get_network_properties_for_graphs(graphs, user, task_name):
    ...
    network_analysis_runnable = SimpletsPropertiesAnalysis(graphs=graphs, user=user, task_name=task_name)
...
class SimpletsPropertiesAnalysis(Runnable):
    ...
    def __initialise_task(cls, task_name, temp, user):
        task = Task()
        task.task_type = SIMPLETS_PROPERTIES_TASK
    ...
    def run(self):
        try:
            ...
            for graph_name, graph_data in self.graphs:
                network = SimpletsPropertiesResult(graph_name=graph_name + str(i), operational_dir=self.operational_directory)
