...
from ClusteringAndEnrichment.DELETE_ClusteringAnalysis import *
...
class EnrichmentAnalysis(ClusteringAnalysis):
    def __init__(self, graph_content, models, task_name, user, distances):
        ClusteringAnalysis.__init__(self, graphs=[[graph_content]], user=user, task_name=task_name, distances=distances)
        self.operational_dir = COMPUTATIONS_DIR + "/" + self.task.operational_directory
        make_system_call("mkdir -p " + self.operational_dir)
        self.task.task_type = ENRICHMENT_ANALYSIS_TASK
        self.task.save()
    ...
    def __get_model_generator(self, model_name):
        # if model_name == "RCC":
        return True
        #  ModelGeneratorALL(number_of_nodes=self.number_of_nodes,
    ...
    def run_task(self):
        try:
            ...
            self.run_analysis()
            # ResultGraphGenerator(task=self.task, models=self.models).generate_graph_images()
