...
from .ProbabilisticNetworksPropertiesResult import ProbabilisticNetworksPropertiesResult
from .settings import COMPUTATIONS_DIR, PROBABILISTIC_NETWORKS_PROPS_TASK, NAMES_MAPPINGS_FILE
...
def get_network_properties_for_graphs(graphs, user, task_name):
    ...
    network_analysis_runnable = ProbabilisticNetworksPropertiesAnalysis(graphs=graphs, user=user, task_name=task_name)
...
class ProbabilisticNetworksPropertiesAnalysis(Runnable):
    ...
    def __initialise_task(cls, task_name, temp, user):
        task = Task()
        task.task_type = PROBABILISTIC_NETWORKS_PROPS_TASK
    ...
    def run(self):
        try:
            ...
            for graph_name, graph_data in self.graphs:
                network = ProbabilisticNetworksPropertiesResult(graph_name=graph_name + str(i), operational_dir=self.operational_directory)
