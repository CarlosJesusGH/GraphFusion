...
def count(G, adj_type):
    ...
    # Re-write code above to work with Python 3.7:
    # if adj_type == 'A1_1':
    #     return format_gradco_output(As_sparse, adj_type, n, order)
    # elif adj_type == 'A1_2':
    #     return format_gradco_output(As_sparse, adj_type, n, order)
    ...
    # elif adj_type == 'A9_11':
    #     return format_gradco_output(As_sparse, adj_type, n, order)
    # elif adj_type == 'A10_10':
    #     return format_gradco_output(As_sparse, adj_type, n, order)
    # elif adj_type == 'A10_11':
    #     return format_gradco_output(As_sparse, adj_type, n, order)
    # elif adj_type == 'A12_12':
    #     return format_gradco_output(As_sparse, adj_type, n, order)
    # elif adj_type == 'A12_13':
    #     return format_gradco_output(As_sparse, adj_type, n, order)
    # elif adj_type == 'A13_13':
    #     return format_gradco_output(As_sparse, adj_type, n, order)
    # elif adj_type == 'A14_14':
    #     return format_gradco_output(As_sparse, adj_type, n, order)
    # elif adj_type == 1:
    #     return compute_graphlet_adjacency_1(As_sparse, n, order)
    # elif adj_type == 2:
    ...
    # Above code can be summarized as:
    cases_str = {'A1_1', 'A1_2', 'A2_1', 'A3_3', 'A4_4', 'A4_5', 'A4_5_bis', 'A5_5', 'A6_7', 'A6_6', 'A8_8', 'A9_10', 'A9_11', 'A10_10', 'A10_11', 'A12_12', 'A12_13', 'A13_13', 'A14_14'}
    if adj_type in cases_str:
        return format_gradco_output(As_sparse, adj_type, n, order)
    elif adj_type >= 1 and adj_type <= 8:
        return eval("compute_graphlet_adjacency_" + str(adj_type))(As_sparse, n, order)
...
def power_iteration(matrix, num_iterations, convergence_threshold=1e-6):
    ...
    for iteration in range(num_iterations):
        ...
        # CGH_added: check for norm of matrix_times_vector is 0
        if np.linalg.norm(matrix_times_vector) == 0:
            print("Poweriteration failed: norm of matrix_times_vector is 0")
            return 0, np.zeros(n)
...
def main():
    ...
    # print("type(centralities)", type(centralities))             # <class 'list'>
    # print("type(centralities[0])", type(centralities[0]))       # <class 'numpy.ndarray'>
    # print("type(centralities[0][0])", type(centralities[0][0])) # <class 'numpy.float64'>
    # print("centralities", centralities)

    # centralities is a list of numpy arrays of floats
    # save centralities to TXT file in current working directory
    # centralities = np.array(centralities)
    # np.savetxt(network_name + "_centralities.txt", centralities, fmt='%f')

    # save centralities to CSV file in current working directory
    centralities = pd.DataFrame(centralities)
    centralities.to_csv(network_name + "_centralities.csv", index=False)
